...
VERSION = "2.2.0"  # Added loading status, memory optimization, enhanced progress
FPS = 24
FRAME_WIDTH = 1920
FRAME_HEIGHT = 1080
...
def compute_motion_score_gpu(prev_frame, current_frame, threshold=30):
    try:
        ...
        prev_resized = cuda.resize(prev_gpu, (int(FRAME_WIDTH * 0.5), int(FRAME_HEIGHT * 0.5)), interpolation=cv2.INTER_AREA)
        curr_resized = cuda.resize(curr_gpu, (int(FRAME_WIDTH * 0.5), int(FRAME_HEIGHT * 0.5)), interpolation=cv2.INTER_AREA)
...
def normalize_frame_gpu(frame, clip_limit=1.0, saturation_multiplier=1.1):
    try:
        ...
        clahe = cv2.createCLAHE(clipLimit=clip_limit, tileGridSize=(8, 8))
        l_clahe = clahe.apply(l_gpu.download())
        l_clahe_gpu = cuda.GpuMat()
        l_clahe_gpu.upload(l_clahe)
        ...
        s_enhanced_gpu = cuda.multiply(s_gpu, np.float32(saturation_multiplier))
        s_enhanced_gpu = cuda.min(s_enhanced_gpu, np.float32(255))
...
def load_video(input_path, cancel_event, progress_callback):
    logging.info("Loading video file")
    cap = cv2.VideoCapture(input_path)
    if not cap.isOpened():
        logging.error("Failed to open video file")
        if progress_callback:
            progress_callback("Loading Failed", 0, 0)
        return None
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    if progress_callback:
        progress_callback("Video Loaded", 10, 0)  # 10% for successful loading
    logging.info(f"Video loaded with {total_frames} frames")
    cap.release()
    return total_frames
...
def detect_motion_segments(input_path, motion_threshold=3000, sample_interval=20, cancel_event=None, progress_callback=None):
    ...
    cap = cv2.VideoCapture(input_path)
    if not cap.isOpened():
        logging.error("Failed to open video file for motion detection")
        if progress_callback:
            progress_callback("Motion Detection Failed", 10, 0)
        return []
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    motion_segments = []
    in_motion = False
    ...
    for frame_idx in range(0, total_frames, sample_interval):
        ...
        if frame_idx % 20 == 0:
            ...
            progress = 10 + (frame_idx / total_frames) * 30  # 10-40% for motion detection
            if progress_callback:
    ...
    for start, end in motion_segments:
        ...
        for _ in range(start, end + 1):
            ...
            if processed_frames % 20 == 0:
                ...
                remaining_time = min(remaining_frames * avg_time_per_frame, 60000)
                progress = 40 + (processed_frames / total_motion_frames) * 30  # 40-70% for intermediate video
                if progress_callback:
    ...
    try:
        ...
        for t in np.arange(0, clip.duration, 1 / FPS):
            ...
            if frame_count % 20 == 0:
                ...
                remaining_time = min(remaining_frames * avg_time_per_frame, 60000)
                progress = 70 + (frame_count / total_frames) * 30  # 70-100% for filtering
                if progress_callback:
...
class VideoProcessorApp:
    ...
    def process_video_thread(self):
        ...
        output_files = {}
        total_frames = load_video(self.input_file, self.cancel_event, self.progress_callback)
        if total_frames is None:
            self.queue.put(("canceled", "Failed to load video file"))
            return
        for task_name, desired_duration in selected_videos:
            ...
            self.queue.put(("task_start", task_name, 10))  # Start at 10% after loading
    ...
    def progress_callback(self, phase, progress, remaining):
        self.queue.put(("progress", phase, progress, remaining))
