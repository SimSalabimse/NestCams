...
# Version number
VERSION = "4.7.0"  # Added preset feature
...
def process_video_multi_pass(input_path, output_path, desired_duration, motion_threshold=3000, sample_interval=5, white_threshold=200, black_threshold=50, clip_limit=1.0, saturation_multiplier=1.1, progress_callback=None, cancel_event=None):
    """Process video in multiple passes: collect all motion frames, filter white/black, trim evenly."""
    ...
    cap.release()
    print(f"Pass 1 complete: Collected {len(include_indices)} frames with motion")
    ...
    if len(filtered_indices) > target_frames:
        ...
        if len(final_indices) < target_frames:
            print(f"Warning: Only {len(final_indices)} frames available, less than desired {target_frames} frames")
...
class VideoProcessorApp:
    def __init__(self, root):
        """Initialize GUI and app state with adjustable settings and presets."""
        self.root = root
        ...
        try:
            ...
            print("No presets found, starting fresh")
    
    def open_settings(self):
        """Open the settings window with sliders and preset management."""
        ...
        # Preset Management
        preset_label = ctk.CTkLabel(self.settings_window, text="Preset Management")
        preset_label.pack(pady=10)
        
        # Load Preset
        load_preset_label = ctk.CTkLabel(self.settings_window, text="Load Preset:")
        load_preset_label.pack(pady=2)
        self.preset_combobox = ctk.CTkComboBox(self.settings_window, values=list(self.presets.keys()))
        self.preset_combobox.pack(pady=2)
        load_button = ctk.CTkButton(self.settings_window, text="Load Selected Preset", command=self.load_preset)
        load_button.pack(pady=5)
        
        # Save Preset
        save_preset_label = ctk.CTkLabel(self.settings_window, text="Save Current Settings as Preset:")
        save_preset_label.pack(pady=2)
        self.preset_name_entry = ctk.CTkEntry(self.settings_window, placeholder_text="Enter preset name")
        self.preset_name_entry.pack(pady=2)
        save_button = ctk.CTkButton(self.settings_window, text="Save Preset", command=self.save_preset)
        save_button.pack(pady=5)
        
        # Save and Reset buttons for main settings
        save_settings_button = ctk.CTkButton(self.settings_window, text="Save Settings", command=self.save_settings)
    ...
    def save_preset(self):
        """Save the current settings as a preset with the given name."""
        preset_name = self.preset_name_entry.get().strip()
        if not preset_name:
            messagebox.showwarning("Warning", "Please enter a name for the preset.")
            return
        if preset_name in self.presets:
            messagebox.showwarning("Warning", "Preset name already exists. Choose a different name.")
            return
        
        self.presets[preset_name] = {
            "motion_threshold": int(self.motion_slider_settings.get()),
            "white_threshold": int(self.white_slider_settings.get()),
            "black_threshold": int(self.black_slider_settings.get()),
            "clip_limit": float(self.clip_slider_settings.get()),
            "saturation_multiplier": float(self.saturation_slider_settings.get())
        }
        ...
        messagebox.showinfo("Info", f"Preset '{preset_name}' saved successfully.")
    ...
    def load_preset(self):
        ...
        if selected_preset in self.presets:
            ...
            self.motion_slider_settings.set(preset["motion_threshold"])
            self.white_slider_settings.set(preset["white_threshold"])
            self.black_slider_settings.set(preset["black_threshold"])
            self.clip_slider_settings.set(preset["clip_limit"])
            self.saturation_slider_settings.set(preset["saturation_multiplier"])
            messagebox.showinfo("Info", f"Preset '{selected_preset}' loaded successfully.")
        else:
            messagebox.showwarning("Warning", "Selected preset not found.")
