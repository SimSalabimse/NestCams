...
# Version number
VERSION = "5.3.0"  # Improved progress feedback
...
def process_video_multi_pass(input_path, output_path, desired_duration, motion_threshold=4000, sample_interval=5, white_threshold=200, black_threshold=50, clip_limit=1.0, saturation_multiplier=1.1, progress_callback=None, cancel_event=None, min_object_area=1000, num_threads=24, start_time=None):
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        for future in as_completed(futures):
            ...
            if progress_callback:
                progress = (completed_futures / num_threads) * 33
                progress_callback(1, progress, completed_futures, num_threads, None)
    ...
    cap = cv2.VideoCapture(input_path)
    filtered_frames = []
    for i, frame_idx in enumerate(include_indices):
        ...
        if progress_callback and i % 10 == 0:
            progress = 33 + (i / len(include_indices)) * 33
            progress_callback(2, progress, i, len(include_indices), None)
    cap.release()
    ...
    for idx in final_indices:
        ...
        if progress_callback and idx % 10 == 0:
            ...
            progress = 66 + (idx / len(final_indices) * 34)
            progress_callback(3, progress, idx, len(final_indices), remaining)
...
class VideoProcessorApp:
    ...
    def process_video_thread(self):
        ...
        for task_name, desired_duration in selected_videos:
            ...
            def progress_callback(stage, progress, current, total, remaining=None):
                percentage = (current / total) * 100 if total > 0 else 0
                self.queue.put(("progress", stage, progress, percentage, remaining))
    ...
    def handle_message(self, message):
        ...
        if message[0] == "task_start":
            ...
            stage, progress_value, percentage, remaining = message[1:]
            self.progress.set(progress_value / 100)
            if stage == 1:
                self.time_label.configure(text="Processing motion detection... (Step 1/3)")
            elif stage == 2:
                self.time_label.configure(text="Filtering frames... (Step 2/3)")
            elif stage == 3:
                if remaining is not None:
                    remaining_min = remaining / 60 if remaining > 0 else 0
                    self.time_label.configure(text=f"Normalizing and writing frames... Est. Time Remaining: {remaining_min:.2f} min ({percentage:.2f}%)")
                else:
                    self.time_label.configure(text="Normalizing and writing frames... (Step 3/3)")
