...
def generate_output_video(input_path, output_path, desired_duration, selected_indices, clip_limit=1.0, saturation_multiplier=1.1, 
                         ...
                         status_callback=None, custom_ffmpeg_args=None, watermark_text=None, pool=None):
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if pool is None:
                pool = Pool(processes=min(os.cpu_count(), 4))
                own_pool = True
            else:
                own_pool = False
...
class VideoProcessorApp:
    ...
    def open_settings(self):
        ...
        self.performance_mode_var = tk.StringVar(value=self.performance_mode)
        ctk.CTkOptionMenu(performance_frame, variable=self.performance_mode_var, 
                         values=["High", "Balanced", "Eco"],
                         command=lambda v: setattr(self, 'performance_mode', v)).pack(side=tk.LEFT)
        ctk.CTkLabel(performance_frame, text="High: Max speed, Balanced: Multitasking, Eco: Background").pack(side=tk.LEFT, padx=5)
