...
def process_video_multi_pass(input_path, output_path, desired_duration, motion_threshold=3000, sample_interval=5, 
                             ...
                             output_format='mp4', progress_callback=None, cancel_event=None, music_paths=None, music_volume=1.0, status_callback=None):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            # Single pass for motion detection and filtering with resized frames
            if status_callback:
                status_callback("Analyzing motion and filtering frames...")
            logging.info("Starting motion detection and filtering pass with resized frames")
            log_session("Starting motion detection and filtering pass with resized frames")
            cap = cv2.VideoCapture(input_path)
            ...
            # Get the appropriate music path
            music_path = music_paths.get(desired_duration, music_paths.get("default"))
