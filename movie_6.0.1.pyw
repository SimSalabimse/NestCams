...
# Version number
VERSION = "6.0.1"  # Updated with preview and start button
...
class VideoProcessorApp:
    def __init__(self, root):
        """Initialize GUI with preview and start button."""
        self.root = root
        ...
        if ret:
            ...
            if normalized is not None:
                if is_white_or_black_frame(normalized, self.white_threshold, self.black_threshold):
                    self.preview_label.configure(text="Frame filtered out (white/black)", image=None)
                else:
